<template>
    <div class="apply-container container">
        ...
        <div class="button-box" v-if="orderDetail.stateCode <= 3">
            <van-button block type="default" @click="CopyOrderChange">复制订单</van-button>
            <van-button block type="info" @click="isCancelVis=true">取消订单</van-button>
        </div>
        <div class="form-button" v-else-if="orderDetail.stateCode == 7">
            <van-button block type="default" @click="CopyOrderChange">复制订单</van-button>

        </div>
        <div class="button-box" v-else-if="orderDetail.stateCode == 6">
            <van-button block type="default" @click="CopyOrderChange">复制订单</van-button>
        ...
        <van-dialog title="是否取消订单" v-model="isCancelVis" show-cancel-button :beforeClose="CancelOrderChange">
            <van-form ref="isCanceForm">
                <van-field class="form-textarea" v-model="cancelReason" :rules="[{required: true}]" name="cancelReason" rows="2" autosize type="textarea" maxlength="50" placeholder="请输入取消原因" show-word-limit />
            </van-form>
        </van-dialog>
